...
module Pagination
  ...
  included do
    ...
    def pagination_for_sent_messages(page_no = 1)
      @pagy, @messages = pagy(ConversationMessage.sent_messages, page: page_no, items: 30)
      [@pagy, @messages]
    end

    def pagination_for_save_messages(page_no = 1)
      @pagy, @messages = pagy(current_profile.saved_items.includes(:profile, :conversation_message).order(:created_at), page: page_no, items: 30)
      [@pagy, @messages]
    end
    ...
    def pagination_for_draft_messages(page_no = 1)
      @pagy, @messages = pagy(current_profile.draft_messages.includes(:profile, :bench_conversation).order(:created_at), page: page_no, items: 30)
      [@pagy, @messages]
    end
  end
end
