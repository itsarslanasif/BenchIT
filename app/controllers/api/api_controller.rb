class Api::ApiController < ApplicationController
  ...
  def set_workspace_in_session
    if session[:current_workspace_id].nil?
      render json: { error_message: 'No Workspace Selected.' }, status: :unprocessable_entity
    else
  ...
  def presence_of_api_token
    ...
    render json: { error_message: 'You need to sign in before to access this.' }, status: :unauthorized
  end
  ...
  def authenticate_api_with_token
    ...
    render json: { error_message: 'You need to sign in before to access this.' }, status: :unauthorized unless current_user
