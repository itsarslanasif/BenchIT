<template>
  <transition name="fade" appear>
    <div
      ...
      <div class="modal rounded-md w-2/5 h-2/5 shadow-md bg-white" role="dialog">
        <div class="p-8">
          ...
          <div class="m-0 relative mt-5">
            <n-form :label-width="80" :model="formValue" :rules="rules">
              <n-form-item :label="$t('workspaces.new_workspace_title')" path="isRequired">
                <n-input v-model:value="formValue.workspaceName" :maxlength="15" :count-graphemes="countGraphemes"
                  show-count :placeholder="$t('workspaces.new_name_placeholder')" />
              </n-form-item>
              <n-form-item class="mt-3" :label="$t('workspace.workspace_type')" path="isRequired">
                <n-select v-model:value="formValue.workspaceType" :options="this.workspaceStoreOptions.workspace_type_options" />
              </n-form-item>
               <n-form-item class="mt-3" label="Please select organization type" path="isRequired">
                <n-select v-model:value="formValue.workspaceOrganizationType" :options="workspaceStoreOptions.organization_type_options" />
              </n-form-item>
               <n-form-item class="mt-3" label="Please enter total workspace capacity" path="isRequired">
                <n-input-number v-model:value="formValue.workspaceCapacity" clearable/>
              </n-form-item>
               <n-form-item label="Enter URL" path="isRequired">
                <n-input v-model:value="formValue.workspaceURL" :maxlength="80" :count-graphemes="countGraphemes"
                  show-count :placeholder="$t('workspaces.new_url_placeholder')" />
              </n-form-item>
...
<script>
import { NForm, NFormItem, NButton, NInput, NSwitch, NSpace, NSelect, NInputNumber } from 'naive-ui';
...
export default {
  ...
  methods: {
    ...
    validations() {
      const { workspaceName, workspaceType, workspaceOrganizationType, workspaceCapacity, workspaceURL } = this.formValue;
      const requiredFields = [workspaceName, workspaceType, workspaceOrganizationType, workspaceCapacity, workspaceURL];
