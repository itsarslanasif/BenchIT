<template>
  <form class="bg-slate-50 rounded-md mx-auto my-6 form text-left p-10" @submit.prevent="handleSubmit()">
    ...
    <div class="btn-div">
      <button v-if="workspaceStore.activePhase === 1" class="bg-slate-50 px-4 py-2 mt-5 border-0">
        {{ $t('workspaces.continue_company_button') }}
      </button>
      <button v-else-if="workspaceStore.activePhase === 2" class="bg-slate-50 px-4 py-2 mt-5 border-0">
        {{ $t('company.continue_url_button') }}
      </button>
      <button v-else-if="workspaceStore.activePhase === 3" class="bg-slate-50 px-4 py-2 mt-5 border-0">
