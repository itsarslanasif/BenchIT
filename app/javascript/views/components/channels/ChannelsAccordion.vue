<template>
  <AccordionList class="my-5 ml-3 text-base text-slate-50">
    <AccordionItem default-opened="true">
      <template #summary>{{ CONSTANTS.CHANNELS }}</template>
      <h5 class=" hover:bg-primaryHover" v-for="channel in this.channels" v-bind:key="channel">
      ...
      <div class="mt-2 -ml-3 hover:bg-primaryHover" @click="modalOpen = !modalOpen">
        <h5 class="pl-3">{{ CONSTANTS.ADD_NEW_CHANNEL }}</h5>
      </div>
    </AccordionItem>
  </AccordionList>
  <div v-if="modalOpen">
...
export default {
  components: { useAccordion, AccordionList, AccordionItem, CreateChannel },
