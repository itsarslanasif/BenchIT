<template>
  <div
    class="flex justify-center items-center z-10 left-0 right-0 top-0 bottom-0 fixed bg-opacity-25 bg-backgroundTransparent overflow-auto"
  >
    <div
      v-click-outside="closeModal"
      class="rounded-md sm:w-flexible-lg md:w-flexible-lg lg:w-150 w-80 shadow-md bg-white"
      role="dialog"
    >
      <div class="p-8">
        <header id="modalTitle" class="flex justify-between">
          <div class="text-lg text-black-900">
            <h1>{{ $t('channels.create_channel') }}</h1>
          </div>
          <button
            type="button"
            class="focus:outline-none bg-none py-1 px-2 text-base float-right text-black-900 rounded"
            @click="closeModal"
          >
