<template>
  ...
  <div class="flex flex-col overflow-auto paneBody">
    <div class="flex items-center justify-center mt-2">
      <img :src="userProfileStore.user_profile?.avatar" alt="user avatar" class="rounded-lg w-90 h-90" />
    </div>
    ...
    <span v-if="userProfileStore.user_profile?.contact_info" class="flex flex-col float-left mx-4 mb-2 text-black-800">
      ...
      <span class="hover-trigger grid grid-cols-8 float-left mt-2 text-black-800">
        ...
        <n-tooltip placement="top" trigger="click">
          <template #trigger>
            <font-awesome-icon @click="copyEmailAddress" icon="fa-copy" class="hover-target col-span-1 self-center text-lg p-2 mr-2 float-right cursor-pointer bg-transparent rounded-md focus:outline-none" />
          </template>
          <span> Copied! </span>
        </n-tooltip>
      </span>
      <span class="hover-trigger grid grid-cols-8 float-left text-lg mt-2 text-black-800">
        ...
        <n-tooltip placement="top" trigger="click">
          <template #trigger>
            <font-awesome-icon @click="copyPhoneNumber" icon="fa-copy" class="hover-target col-span-1 self-center text-lg p-2 mr-2 float-right cursor-pointer bg-transparent rounded-md focus:outline-none" />
          </template>
