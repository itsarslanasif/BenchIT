<template>
  <div class="bg-white flex flex-col p-5 gap-3">
    <div class="bg-white hover:bg-transparent p-2 rounded-md">
      <p class="font-black">{{ $t('channel_detail.topic') }}</p>
      <p>{{ topic }}</p>
    </div>
    <div class="bg-white hover:bg-transparent p-2 rounded-md">
      <p class="font-black">{{ $t('channel_detail.description') }}</p>
      <p>{{ conversationInfo.description }}</p>
    </div>
    <div class="bg-white hover:bg-transparent p-2 rounded-md">
      <p class="font-black">{{ $t('channel_detail.created_by') }}</p>
      <p>{{ channelCreator }} </p>
    </div>
    <div
      ...
      @click="leaveChannel"
    >
      <p class="font-black">{{ $t('channel_detail.leave') }}</p>
    </div>
    <div class="bg-white hover:bg-transparent p-2 rounded-md">
      <p class="font-black">{{ $t('channel_detail.files') }}</p>
      <p>{{ $t('channel_detail.filecontent') }}</p>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const channelCreator = computed(() => {
      return `${conversationInfo.value.creator_name} on ${date.value}`
    })
