<template lang="">
  <div class="p-3">
    <div v-if="list.length" v-for="item in list" class="my-4">
      ...
      <div class="flex gap-3 bg-slate-50 border border-black-300 rounded p-2" v-if="listType === $t('search_bar.profiles')" @click="goToChat(`/profiles/${item.id}`)"  >
        <div class="align-baseline">
      ...
      <div class="flex gap-3 bg-slate-50 border border-black-300 rounded p-2" v-if="listType === $t('search_bar.channels')" @click="goToChat(`/channels/${item.id}`)" >
        <div class="bg-slate-100 w-12 h-12 rounded flex items-center justify-center text-xl">
