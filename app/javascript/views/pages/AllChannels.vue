  ...
  <div class="screen-height">
    ...
    <div class="px-5 py-3">
      <div
        ...
        v-for="channel in channels.query"
        :key="channel.id"
      >
        <div class="w-5/6 px-2 py-3 font-bold	relative">#{{ channel.name }}</div>
        <div class="absolute px-5 py-8 font-thin">{{channel.description}}</div>
        <div class="py-3" @click="joinChannel(channel.id)" v-if="showButton"><n-button type="success">{{$t('actions.join')}}</n-button></div>
      </div>
    </div>
  </div>
</template>
