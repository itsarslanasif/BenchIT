<template>
  <div>
    <div v-if="conversation_type && id" class="flex bg-white flex-col h-full">
      ...
      <div class="px-3 editor-style text-black-900" v-if="isMember">
        <TextEditorVue
...
export default {
  ...
  setup(props) {
    ...
    unreadStore.markedChatAsRead(props.conversation_type, props.id);
    return {
      messages,
      currMessage,
      currentPage,
      ...
      loadMoreMessages() {
        if (hasMoreMessages) {
          messageStore.index(props.conversation_type, props.id);
        }
      },
      messageStore,
      currentUser,
